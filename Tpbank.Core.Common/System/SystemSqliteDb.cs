...
namespace Tpbank.Core.Common.System
{
    public class SystemSqliteDb
    {
        ...
        public SystemSqliteDb(string systemDataFile)
        {
        ...
        {
            return string.Format("Data Source={0};Version=3;New=False;Compress=True; datetimeformat=CurrentCulture", systemDataFile);
        }

        public void InsertHistory()
        {   
            using (SQLiteConnection sql_con1 = new SQLiteConnection(getConnectionString()))
            {
                sql_con1.Open();
                using (var transaction = sql_con1.BeginTransaction())
                {
                    SQLiteCommand insertCommand = sql_con1.CreateCommand();
                    ...
                    SQLiteCommand selectCommand = sql_con1.CreateCommand();
